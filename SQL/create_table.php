...
<body>
	...
	//connect to sql server and locate database
	if ($dbc = @mysql_connect('localhost', 'root', 'skateboard')) {
		...
		print '<p style="colord: red;">could not connect to MySQL:<br />' . mysql_error() . '.</p>';
	}
	// define query for creating the table
	if ($dbc) {
		$query = 'CREATE TABLE entries (entry_id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY, title VARCHAR(100) NOT NULL, entry TEXT NOT NULL, date_entered DATETIME NOT NULL)';
	}
